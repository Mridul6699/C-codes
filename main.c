/*
Project Name:Vaccination Registration System
High On Codes --~
Manik Singh(105),Mridul Krishna(104),Sujal Magotra(109).
*/
...
// Global Variables
int n;
char Union_Territory [20], dis[20], hos[40], date[12], hour[6];
...
void venue()
{
	...
	// Calling Heading() Function
	heading();
	printf("\t\t\t\tEnter Union Territory: ");
	gets(Union_Territory);
...
void receipt()
{
	...
	printf("\t\t\t\tUnion Territory: ");
	puts(Union_Territory);
	printf("\t\t\t\tDistrict: ");
	puts(dis);
	printf("\t\t\t\tDate: ");
	puts(date);
